...
//#define TicksForMillis (BSP_HOCO_HZ / 1000)
#define TicksForMillis (g_timer0_cfg.period_counts)
...
static inline unsigned long timerTicks()
{
    ...
    unsigned long cmcnt = g_timer0_ctrl.p_reg->GTCNT;
    interrupts();
...
{
    unsigned long s = g_timer0_ctrl.p_reg->GTCNT;
    for (;;) {
        unsigned long l = g_timer0_ctrl.p_reg->GTCNT;
        unsigned long d = l - s;
        if (l < s) {
            d += TicksForMillis;
        }
...
unsigned long micros() {
    ...
    unsigned long cmcnt = g_timer0_ctrl.p_reg->GTCNT;
    interrupts();
    return 1000 * ms + cmcnt * 1000 / TicksForMillis;
#endif
}
