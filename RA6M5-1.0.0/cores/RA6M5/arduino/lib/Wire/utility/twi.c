...
void twi_disable(void)
{
  ...
  digitalWrite(SCL, 0);
#else
  R_IIC_MASTER_Close(&g_i2c_master1_ctrl);
#endif
}
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
  ...
  R_IIC_MASTER_SlaveAddressSet (&g_i2c_master1_ctrl, address, I2C_MASTER_ADDR_MODE_7BIT);
  R_IIC_MASTER_Read(&g_i2c_master1_ctrl, data, length, !sendStop);
...
uint8_t twi_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t wait, uint8_t sendStop)
{
  ...
  R_IIC_MASTER_SlaveAddressSet (&g_i2c_master1_ctrl, address, I2C_MASTER_ADDR_MODE_7BIT);
  err = R_IIC_MASTER_Write(&g_i2c_master1_ctrl, data, length, !sendStop);
  ...
  if (err == FSP_ERR_IN_USE){
      return 2;
  } else if (I2C_MASTER_EVENT_ABORTED == g_i2c_callback_event)
  {
      return 4;
  }
  return 0;
